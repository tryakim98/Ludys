...
| GAP-014 | Provider-/emulator-/staginggrenser | OWNER_DECISION_RECORDED_TECHNICAL_STAGING_NOT_OPENED | WP13.12A | Produkteier har eksplisitt valgt `APPROVE_RECOMMENDED_SYNTHETIC_DEV`: `FIREBASE_CAPABILITY`, `europe-north1` som kandidat uten regionlås og funksjonsutstedt kortlivet capability. Bare isolert og tidsbegrenset syntetisk staging er besluttet. WP13.12B krever separat implementeringsprompt og skal stoppe etter dokumentert teknisk stagingproof. Provideraktivering, WP13.12B, B8, studentbeta, rekruttering, ekte data og produksjon er fortsatt blokkert; cloud resources er 0; kostgrenser, driftsansvar og stagingutløp må avklares før provisioning. |
