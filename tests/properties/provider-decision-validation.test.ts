...
const cases: readonly [string, (pkg: MutablePackage) => void, RegExp][] = [
  ...
  ["missing explicit owner confirmation", (pkg) => { (pkg.ownerDecisionRecord.signatureOrExplicitOwnerConfirmation as unknown as { explicitOwnerConfirmationPresent: boolean }).explicitOwnerConfirmationPresent = false; }, /owner confirmation/],
  ["wrong selected provider", (pkg) => { (pkg.ownerDecisionRecord as unknown as { selectedOption: string }).selectedOption = "FIREBASE_ANONYMOUS"; }, /selection/],
  ["missing approved boundary", (pkg) => { (pkg.ownerDecisionRecord as unknown as { conditions: string[] }).conditions = pkg.ownerDecisionRecord.conditions.slice(1); }, /conditions/],
  ["WP13.12B opened by decision record", (pkg) => { (pkg.ownerDecisionRecord.effects as unknown as { opensWp13_12b: boolean }).opensWp13_12b = true; }, /effects opened/],
