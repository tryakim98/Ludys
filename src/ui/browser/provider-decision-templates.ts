...
export function renderProviderDecision(view: ProviderDecisionView): string {
  ...
  return `<a class="skip-link" href="#provider-decision-main">${escapeHtml(nb ? "Hopp til hovedinnholdet" : "Hopp til hovudinnhaldet")}</a>
    ...
    <div class="classification-banner decision-boundary">${escapeHtml(nb
      ? "Produkteier har godkjent den anbefalte syntetiske dev-retningen med bindende grenser. WP13.12B er ikke åpnet, og ingen skyressurs er opprettet."
      : "Produkteigar har godkjent den tilrådde syntetiske dev-retninga med bindande grenser. WP13.12B er ikkje opna, og ingen skyressurs er oppretta.")}</div>
    <main id="provider-decision-main" class="decision-page" tabindex="-1">
      ...
      <section data-decision-section="data" aria-labelledby="decision-data-title"><h2 id="decision-data-title">${escapeHtml(label("data"))}</h2>
        <p>${escapeHtml(nb ? "Tillatte kandidater er kun syntetiske og betinget av en separat WP13.12B-implementeringsprompt. Forbudte klasser er NOT_COLLECTED og NOT_AUTHORIZED." : "Tillatne kandidatar er berre syntetiske og avhengige av ein separat WP13.12B-implementeringsprompt. Forbodne klassar er NOT_COLLECTED og NOT_AUTHORIZED.")}</p>
      ...
      <section data-decision-section="sources" aria-labelledby="decision-sources-title"><h2 id="decision-sources-title">${escapeHtml(label("sources"))}</h2><p>checkedAt ${escapeHtml(view.officialSources[0]?.checkedAt ?? "UNKNOWN")} · ${view.officialSources.length} ${escapeHtml(nb ? "primærkilder" : "primærkjelder")}</p><ol>${view.officialSources.map((source) => `<li><strong>${escapeHtml(source.title)}</strong> · ${escapeHtml(source.publisher)}<br><code>${escapeHtml(source.officialUrl)}</code></li>`).join("")}</ol></section>
      <section data-decision-section="owner" class="decision-owner" aria-labelledby="decision-owner-title"><h2 id="decision-owner-title">${escapeHtml(label("owner"))}</h2><p>${escapeHtml(view.ownerDecisionRecord.decision)} · ${escapeHtml(view.ownerDecisionRecord.decisionDate)} · SIGNATURE = false</p>${facts([["Selected option", view.ownerDecisionRecord.selectedOption], ["Selected region", `${view.ownerDecisionRecord.selectedRegion} · ${view.ownerDecisionRecord.selectedRegionStatus}`], ["Capability", view.ownerDecisionRecord.selectedCapabilityModel], ["WP13.12B opened", String(view.ownerDecisionRecord.effects.opensWp13_12b)], ["Next scope", view.ownerDecisionRecord.effects.nextAuthorizedScope]])}${list(view.ownerDecisionRecord.conditions)}<div class="button-row">${button("export-dossier", nb ? "Eksporter beslutningsdossier" : "Eksporter avgjerdsdossier")}${button("export-owner-template", nb ? "Eksporter blank produkteiermal" : "Eksporter blank produkteigarmal")}</div>${view.dossierPreview === "" ? "" : `<a id="provider-dossier-download" download="ludys-wp13-12a-decision-dossier.json">${escapeHtml(nb ? "Lagre dossier" : "Lagre dossier")}</a><textarea id="provider-dossier-preview" readonly>${escapeHtml(view.dossierPreview)}</textarea>`}${view.ownerTemplatePreview === "" ? "" : `<a id="provider-owner-template-download" download="WP13_12A_OWNER_DECISION_TEMPLATE.md">${escapeHtml(nb ? "Lagre blank mal" : "Lagre blank mal")}</a><textarea id="provider-owner-template-preview" readonly>${escapeHtml(view.ownerTemplatePreview)}</textarea>`}</section>
      <section class="operations-terminal"><h2>${escapeHtml(nb ? "Eksplisitt stoppunkt" : "Eksplisitt stoppunkt")}</h2><p>OWNER_DECISION = ${escapeHtml(view.authorization.ownerDecision)} · PROVIDER_ACTIVATION = BLOCKED · CLOUD_RESOURCES = 0 · WP13.12B = BLOCKED · STUDENT_BETA = NOT_AUTHORIZED · PRODUCTION = NOT_AUTHORIZED</p></section>
    </main>
