...
export function renderExerciseReview(packet: ExerciseReviewPacket, locale: Locale, selectedId: string, notes: readonly ExerciseReviewNote[] = [], drafts: ReadonlyMap<string, ReviewNoteDraft> = new Map()): string {
  ...
  return `<div class="exercise-room exercise-review" data-view-key="review-${locale}-${selected?.exerciseId ?? "empty"}"><a class="skip-link" href="#exercise-main">${t("Hopp til innhold", "Hopp til innhald")}</a>
    <header class="site-header exercise-header"><div class="exercise-identity">${skynjaBrand()}<span class="exercise-location">${t("Innholdsgjennomgang", "Innhaldsgjennomgang")}</span></div><button type="button" data-exercise-action="review-close">${t("Til øvelsene", "Til øvingane")}</button></header>
    ...
    <section class="exercise-review-export" aria-labelledby="exercise-review-export-title"><h2 id="exercise-review-export-title">${t("Gjennomgang og arbeidsnotater", "Gjennomgang og arbeidsnotat")}</h2><details class="review-materials"><summary>${t("Last ned gjennomgangspakken", "Last ned gjennomgangspakken")}</summary>
      ...
      </details><h3>${t("Dine arbeidsnotater", "Arbeidsnotata dine")}</h3><p>${t("Notater er forslag og observasjoner. De endrer ikke innholdet eller godkjenningsstatusen. Lagre filen i avtalt kanal.", "Notat er forslag og observasjonar. Dei endrar ikkje innhaldet eller godkjenningsstatusen. Lagre fila i avtalt kanal.")}</p>
