import { skynjaBrand } from "./skynja-home-templates.js";
...
function pendingReview(view: AuthoringPipelineView): string {
  ...
  return `<section data-content-proposal data-copy-human-reviewed="false" aria-labelledby="proposal-heading">
    ...
    <p>${nn ? "Tekst, ordval og lydmanus er ikkje menneskeleg godkjende. Pakken kan redigerast og eksporterast her i verkstaden." : "Tekst, ordvalg og lydmanus er ikke menneskelig godkjent. Pakken kan redigeres og eksporteres her i verkstedet."}</p>
...
export function renderAuthoringWorkspace(view: AuthoringPipelineView): string {
  ...
  return `<a class="skip-link" href="#authoring-main">Hopp til innholdsverkstedet</a>
    <header class="site-header app-header authoring-header"><div>${skynjaBrand()}<h1>Innholdsverksted</h1></div><button type="button" data-authoring-action="close">Til startsiden</button></header>
    <main id="authoring-main" class="authoring-workspace" tabindex="-1" data-copy-human-reviewed="${String(AUTHORING_UI_COPY_REVIEW.humanReviewed)}" data-copy-review-source="${AUTHORING_UI_COPY_REVIEW.reviewSource}">
      <section class="authoring-boundary" aria-labelledby="authoring-title"><h2 id="authoring-title" tabindex="-1">Tekst, støtte og lyd</h2><p>Innholdet er et utkast. Faglig og språklig gjennomgang gjenstår, og publisering er stengt.</p><details><summary>Versjon og teknisk status</summary><div class="status-cluster" role="status"><span>DRAFT</span><span>EXTERNAL_REVIEW_REQUIRED</span><span>0 EXTERNAL RECEIPTS</span><span>PUBLISHING BLOCKED</span><span>NOT STUDENT BETA</span></div><p><strong>AUTHORING_PIPELINE_READY · AUDIO_PRODUCTION_PIPELINE_READY · SYNTHETIC_ONLY</strong></p></details><p>Læringsdimensjonene beskriver aktivitetens oppgavekrav. De er ikke dysleksityper, og appen diagnostiserer eller profilerer ikke barnet.</p></section>
      <section class="authoring-selector" aria-labelledby="package-heading"><h2 id="package-heading">Velg pakke</h2><label for="authoring-package-select">${view.packages.length} pakker og lokale utkast</label><select id="authoring-package-select">${view.packages.map((item) => `<option value="${escapeHtml(item.packageId)}" ${item.selected ? "selected" : ""}>${escapeHtml(item.title)} · ${escapeHtml(item.lifecycle)}</option>`).join("")}</select><details><summary>Om innholdspakken</summary><dl class="authoring-facts"><div><dt>Aktivitet</dt><dd>${escapeHtml(authoringPackage.activityId)}</dd></div><div><dt>Mønsterklasse</dt><dd>${escapeHtml(authoringPackage.patternClassId)}</dd></div><div><dt>Oppgaven undersøker</dt><dd>${escapeHtml(authoringPackage.construct)}</dd></div><div><dt>Gjennomgang</dt><dd>${escapeHtml(authoringPackage.reviewStatus)}</dd></div><div><dt>Innholdsstatus</dt><dd>${escapeHtml(authoringPackage.lifecycle)}</dd></div></dl></details><div class="button-row" aria-label="Velg målform"><button type="button" data-authoring-locale="nb-NO" aria-pressed="${String(view.locale === "nb-NO")}">BM</button><button type="button" data-authoring-locale="nn-NO" aria-pressed="${String(view.locale === "nn-NO")}">NN</button></div></section>
      ${pendingReview(view)}
      <section aria-labelledby="locale-fields-heading"><h2 id="locale-fields-heading">${view.locale === "nb-NO" ? "Bokmål" : "Nynorsk"} – eksplisitt redaksjonell variant</h2><div class="authoring-form-grid">${field("Tittel", "title", copy.title)}${field("Målord", "targetWord", copy.targetWord)}${field("Neste ord", "transferWord", copy.transferWord)}${field("Instruksjon", "instruction", copy.instruction, true)}${field("Spørsmål om betydning", "meaningPrompt", copy.meaningPrompt, true)}${field("Spørsmål til videreføring", "transferPrompt", copy.transferPrompt, true)}${field("Voksen – forstå", "adultUnderstand", copy.adultCard.understand, true)}${field("Voksen – gjør eller si", "adultDoOrSay", copy.adultCard.doOrSay, true)}${field("Voksen – unngå", "adultAvoid", copy.adultCard.avoid, true)}${field("Voksen – fordyp deg", "adultDeepen", copy.adultCard.deepen, true)}${field("Kunnskapstittel", "knowledgeTitle", copy.knowledgeUnit.title)}${field("Kunnskapsforklaring", "knowledgeExplanation", copy.knowledgeUnit.explanation, true)}${field("Situasjonskort", "contextCard", copy.contextCard.text, true)}</div></section>
