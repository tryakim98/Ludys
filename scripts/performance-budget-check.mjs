...
const policy = JSON.parse(await readFile(join(root, "release", "wp13-12a", "performance-budgets.json"), "utf8"));
...
const stateModule = await import(pathToFileURL(join(root, "dist", "src", "content", "prototype", "wp13-12a-release-state.js")));
const started = performance.now();
for (let index = 0; index < 1000; index += 1) {
  const result = releaseModule.rollbackComponent(stateModule.wp13_12aLocalReleaseState, "AUDIO", "TEXT_AND_SILENCE", "2026-07-23T12:00:00.000Z");
...
console.log(`WP13.12A performance budgets passed without weakening prior limits: ${JSON.stringify({ ...measurements, rollbackResponseMs })}`);
