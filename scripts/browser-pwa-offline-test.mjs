...
try {
  ...
  const cacheProof = await evaluate(`(async () => {
    ...
    const name = names.find((candidate) => candidate === 'ludys-shell-0.14.0-reconstructed.9-skynja-evidence-2026-09-23');
    if (!name) return { names, urls: [] };
    const cache = await caches.open(name);
    return { names, urls: (await cache.keys()).map((request) => new URL(request.url).pathname) };
  })()`);
  assert.ok(cacheProof.names.includes("ludys-shell-0.14.0-reconstructed.9-skynja-evidence-2026-09-23"));
