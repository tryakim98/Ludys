import { evidenceAfterSupport, } from "./evidence.js";
...
function update(state, patch, events) {
    return {
    ...
    switch (command.kind) {
        ...
        case "WAIT":
            return isActivePhase(state.phase)
                ? update(state, { phase: "WAITING_WITHOUT_INTERVENTION", activeCard: undefined }, [
        ...
        case "SHOW_ADULT_CARD":
            ...
            return update(state, { phase: "ADULT_CONSIDERING", activeCard: command.card }, [
        ...
        case "ADULT_WAIT":
            return state.phase === "ADULT_CONSIDERING"
                ? update(state, { phase: "WAITING_WITHOUT_INTERVENTION", activeCard: undefined }, [
                    ...(state.activeCard
                        ? [{ type: "ADULT_CARD_DISMISSED", cardId: state.activeCard.cardId }]
                        : []),
                    { type: "STATE_CHANGED", to: "WAITING_WITHOUT_INTERVENTION" },
                    { type: "NO_INTERVENTION" },
                ])
        ...
        case "REQUEST_QUIET": {
            const stoppedAudio = state.playingAudioIds.map((audioSpecId) => ({ type: "AUDIO_STOPPED", audioSpecId }));
            return isActivePhase(state.phase)
                ? update(state, { quietMode: true, playingAudioIds: [] }, [{ type: "QUIET_MODE_ENABLED" }, ...stoppedAudio])
        ...
        case "REPLAY_AUDIO":
            ...
            return update(state, { playingAudioIds: [command.audioSpecId] }, [{ type: "AUDIO_REQUESTED", audioSpecId: command.audioSpecId }]);
        case "PAUSE": {
        ...
        case "RESUME":
            return state.phase === "PAUSED" && state.previousActivePhase !== undefined
                ? update(state, { phase: state.previousActivePhase, previousActivePhase: undefined }, [{ type: "STATE_CHANGED", to: state.previousActivePhase }])
        ...
        case "COMPLETE":
            return state.phase === "CHILD_ACTING" ||
                ...
                ? update(state, { phase: "COMPLETED", activeCard: undefined, playingAudioIds: [] }, [{ type: "STATE_CHANGED", to: "COMPLETED" }])
        ...
        case "RECONNECT":
            ...
            return update(state, { activeCard: undefined, playingAudioIds: [] }, [
                ...(state.activeCard
                    ? [{ type: "ADULT_CARD_DISMISSED", cardId: state.activeCard.cardId }]
                    : []),
                ...state.playingAudioIds.map((audioSpecId) => ({ type: "AUDIO_STOPPED", audioSpecId })),
                { type: "RECONNECTED" },
            ]);
    }
}
