...
import { applySyntheticStagingCommand, createSyntheticStagingAggregate, deleteSyntheticStagingAggregate, projectSyntheticStaging, SYNTHETIC_STAGING_RELEASE_ID, } from "../../../../src/core/synthetic-staging.js";
export const CAPABILITY_VERSION = "wp13.12b-capability-v1";
export const CAPABILITY_MAX_LIFETIME_MS = 15 * 60 * 1000;
export const DEFAULT_COMMAND_BUDGET = 64;
export const EXTERNAL_STAGING_EXPIRY_AT = "2027-01-25T00:00:00.000Z";
...
function accepted(value, status = 200) {
    return { ok: true, status, denialClass: undefined, value };
}
function denied(status, denialClass) {
    return { ok: false, status, denialClass, value: undefined };
}
function containsProhibitedCapabilityField(value) {
    if (value === null || typeof value !== "object")
        return false;
    if (Array.isArray(value))
        return value.some(containsProhibitedCapabilityField);
    return Object.entries(value).some(([key, nested]) => (PROHIBITED_CAPABILITY_KEYS.has(key) || containsProhibitedCapabilityField(nested)));
}
...
export class SyntheticStagingAuthoritativeHandler {
    ...
    releaseIds;
    options;
    constructor(store, hmacKey, releaseIds, options) {
        this.store = store;
        this.hmacKey = hmacKey;
        this.releaseIds = releaseIds;
        this.options = options;
        if (hmacKey.byteLength < 32)
            throw new Error("CAPABILITY_HMAC_KEY_TOO_SHORT");
        if (!validProjectId(options.projectId))
            throw new Error("INVALID_SYNTHETIC_PROJECT_ID_FORMAT");
    }
    #now() {
        return this.options.now?.() ?? new Date().toISOString();
    }
    #externalStagingTermExpired(now) {
    ...
    #verifyCapability(token) {
        ...
        try {
            ...
            if (parsed.capabilityVersion !== CAPABILITY_VERSION
    ...
    #bearer(request) {
        if (containsProhibitedCapabilityField(request.body)
    ...
    async #authorize(request, expectedSessionId) {
        const now = this.#now();
        ...
        if (token === undefined)
            return denied(401, "BEARER_ONLY");
        const claims = this.#verifyCapability(token);
        if (claims === undefined)
            return denied(401, "CAPABILITY_INVALID");
        if (parseIso(claims.expiresAt) === undefined
            || parseIso(claims.issuedAt) === undefined
            || Date.parse(claims.expiresAt) <= Date.parse(now))
            return denied(401, "CAPABILITY_EXPIRED");
        if (claims.sessionBinding !== expectedSessionId)
            return denied(403, "SESSION_BINDING_MISMATCH");
        const control = await this.store.loadControl();
        if (!control.stagingEnabled)
            return denied(503, "KILL_SWITCH_ACTIVE");
        if (claims.controlEpoch !== control.controlEpoch)
            return denied(401, "CONTROL_EPOCH_STALE");
        const tombstone = await this.store.loadTombstone(expectedSessionId);
        if (tombstone !== undefined)
            return denied(410, "TOMBSTONE");
        const aggregate = await this.store.loadSession(expectedSessionId);
        if (aggregate === undefined)
            return denied(404, "SESSION_NOT_FOUND");
    ...
    async issueSyntheticSession(input) {
        ...
        if (!this.options.sessionIssuanceEnabled)
            return denied(503, "SESSION_ISSUANCE_DISABLED");
        if (this.options.runtimePhase === "EXTERNAL_SYNTHETIC_STAGING"
            && !this.options.previewIngressReady)
            return denied(503, "PREVIEW_INGRESS_NOT_READY");
        if (!input.operatorAuthorized)
            return denied(403, "OPERATOR_AUTHORIZATION_REQUIRED");
        const control = await this.store.loadControl();
        if (!control.stagingEnabled)
            return denied(503, "KILL_SWITCH_ACTIVE");
    ...
    async sessionCommand(request) {
        const authorized = await this.#authorize(request, request.body.syntheticSessionId);
        if ("ok" in authorized)
            return authorized;
        if (authorized.grant.remainingCommands < 1)
            return denied(429, "COMMAND_BUDGET_EXCEEDED");
    ...
    async setKillSwitch(input) {
        ...
        if (!/^[A-Z0-9_]{3,64}$/.test(input.reasonCode))
            return denied(400, "REASON_CODE_INVALID");
    ...
    async health() {
        const control = await this.store.loadControl();
        const stagingTermExpired = this.#externalStagingTermExpired(this.#now());
        const ingressReady = (this.options.runtimePhase === "LOCAL_EMULATOR_PROOF"
            || this.options.previewIngressReady);
        const externallyActive = (this.options.runtimePhase === "EXTERNAL_SYNTHETIC_STAGING"
            && !stagingTermExpired
            && this.options.sessionIssuanceEnabled
            && control.stagingEnabled
            && ingressReady);
        return accepted({
            serviceHealth: (!stagingTermExpired
                && this.options.sessionIssuanceEnabled
                && control.stagingEnabled
                && ingressReady) ? "READY" : "READY_DISABLED_BY_DEFAULT",
